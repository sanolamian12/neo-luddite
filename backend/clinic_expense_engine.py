...
def rule_복리후생비(p: ClinicProfile, e: ExpenseInput):
    # 헬스장 사안에서 출발한 규칙이라 "회원권 명의"가 모든 복리후생비에 붙어 있었다 — 명절 떡값(현금)에도
    # 회원권 명의를 요구해 세무사가 '환각 증빙'으로 지적(2026-09-18 확인). 엔진은 현금·시설이용을 가르는
    # 입력이 없어서 지급 형태별 증빙은 조건을 붙여 둘 다 싣는다(현금 떡값 = 근로소득 원천징수 대상).
    ev = ["복리후생 사내규정", "지급·이용 대장(대상·금액·시기)",
          "현금·상품권 지급 시 근로소득 원천징수 내역", "시설·회원권 이용 시 사업자 명의 계약"]
    # 근거 문구는 판정 카드 요약(summary)이자 작문 프롬프트의 '근거'다. 예전 "→ 복리후생비 인정"은 계정을 단정해,
    # 현금 떡값처럼 시행령 §45 열거 밖인 지급에서 본문("복리후생비가 아닌 급여성 상여", 드물게 "손금 불인정")과
    # 카드가 어긋났다(2026-09-18 스모크). 비용 인정 여부는 계정(복리후생비·급여)과 무관하게 같고, 현금·상품권이면
    # 직원 쪽은 근로소득 원천징수다 — 판정·금액은 그대로, 문구만 맞는 쪽(비용 전액 인정)으로.
    if e.전직원_수혜 and e.사규근거:
        return ExpenseResult(Verdict.전부인정, e.amount, 35,
            "전 직원 대상 + 사내 복리후생 규정 근거 → 지출액 전액 비용 인정(필요경비·손금). 현금·상품권 지급이면 "
            "세무상 직원의 근로소득(급여성 상여)이라 원천징수 대상이며, 복리후생비·급여 중 어느 계정이든 비용 인정은 같다", ev)
    return ExpenseResult(Verdict.부인, 0, 85,
