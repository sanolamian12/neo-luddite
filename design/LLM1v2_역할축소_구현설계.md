...
- 대가: **정보를 다 준 사용자도 1턴은 질문을 받는다** — 경험 저하, 고장 아님. 완화책은 §6 O-1.
...
| **3 (W4 축소판)** | §3 흐름 + D4 카운터 + `check_asked()` + D3 대체 질문 + `is_stalled` 제외 | 3턴 시나리오(부분답변·모르겠어요·상한) 로컬 통과 |
...
- **O-1 1턴 되묻기 UX**: 정보를 다 준 사용자도 질문을 받는다. 되묻기 카드에 "이대로 답변 받기"(→ `proceed_with_gap`)를 둘지.
  정본 A 계열에 없다 → 정본 추가 여부부터.
