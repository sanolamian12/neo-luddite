...
4. **구독 준비 틈 전면 해소 + 로딩 스피너** (사용자 요청 2026-09-21 — (b)에서 남긴 §7 미결을 이렇게 정했다)
   - `makeCollectionSync` 를 쓰는 나머지 컬렉션 전부(대화·신청·메일·일감·검수·리뷰·이의·원장·정산·평가자 명부 등 11개,
     `lib/*-store.ts`)에 `waitForPostgresReady: true` 를 켠다 — 새로 붙이는 `consultation_offers` 도 처음부터 켠다.
     그 대가로 첫 적재가 약 2.5초(새 페이지에선 최대 5초 타임아웃) 늦어진다.
   - 그 대기를 **동그라미 스피너**로 보여 준다(진행률 막대 아님 — 사용자: "동그라미 형태면 충분").
     공용 컴포넌트 `components/ui/spinner.tsx` 를 새로 만들고(lucide `LoaderCircle` + `animate-spin`, `role="status"`·`aria-label`,
     `prefers-reduced-motion` 이면 회전 멈춤), 지금 흩어진 **"불러오는 중…" 문구 20곳(16개 파일)** 을 스피너 + 문구로 바꾼다
     (`grep -rn "불러오는 중" frontend/components frontend/app`). 사이드바 상담 세션 목록·`[채팅방 열기]` 대기 버튼 같은 작은 자리는 작은 크기로.
   - 스피너가 **영원히 도는 자리가 없어야 한다** — 적재 실패도 `onHydrated` 로 끝나는 기존 3중 대응(타임아웃·재시도·복귀 재시도)을 그대로 믿되,
     E2E 로 확인한다. 켠 뒤 **첫 화면이 뜨기까지의 시간을 켜기 전·후로 실측**해 §7 에 적는다.
...
- 스피너·준비 대기: 로그인 직후 사장님·세무사·admin 첫 화면에서 스피너가 떴다가 사라짐, 영원히 도는 자리 0,
  **페이지를 연 지 3초 안에 다른 컨텍스트가 만든 행(신청·메일)이 유실되지 않음**(방 E2E 의 수락 직후 시나리오를 신청·메일에도),
  첫 화면 표시 시간 켜기 전·후 실측. 412px 에서도 스피너 자리가 가로 스크롤을 만들지 않음.
