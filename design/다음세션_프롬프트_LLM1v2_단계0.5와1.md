...
- 열린 결정 **O-2(턴 간 상태)** 를 여기서 확정 — `schema.py`·프론트 `Message` 타입을 보고 권고안(응답 선택 필드 + history 왕복)이
  맞는지 확인, 사용자에게 한 줄로 확인받는다.
- 완료 조건: 디폴트 v1 에서 기존 테스트 통과 · `?pipeline=v2` 가 골격을 탄다.

여력이 남으면 단계 2(W3, `llm1.extract_facts()` — 부록 A **원문 그대로**, 판정 필드 값은 무시)에 착수.

═══ 사용자에게 물을 것 (정하지 말 것) ═══
- **O-1**: 1턴 되묻기 카드에 "이대로 답변 받기"(→ proceed_with_gap)를 둘지. 정본 A 계열에 없다 → 정본 추가 여부부터.
- **O-2**: 위(단계 1 에서 근거를 보고 확인받기).
